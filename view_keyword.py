...
from ui import render_empty_state, render_kpi_strip, render_ops_cards, render_toolbar, safe_numeric_col
...
def _render_export_buttons(df: pd.DataFrame, filename_prefix: str, key_prefix: str) -> None:
    if df is None or df.empty:
        return
    clean = df.copy()
    today = pd.Timestamp.today().strftime("%Y%m%d")
    prefix = f"{filename_prefix}_{today}"
    ...
    c1.download_button(
        ...
        mime="text/csv",
        key=f"{key_prefix}_csv_{len(clean)}_{len(clean.columns)}",
    )
    c2.download_button(
        "엑셀 다운로드",
        data=_make_xlsx_bytes(clean, filename_prefix[:31]),
        file_name=f"{prefix}.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        key=f"{key_prefix}_xlsx_{len(clean)}_{len(clean.columns)}",
    )
...
def _add_perf_metrics(view: pd.DataFrame) -> pd.DataFrame:
    for c in ["광고비", "전환매출", "구매완료 매출", "노출", "클릭", "전환", "구매완료"]:
...
def _prefer_total_conversion_for_keyword(df: pd.DataFrame) -> pd.DataFrame:
    """Expose both total conversions and purchase-complete conversions.

    `conv/sales` are used by the existing UI as total conversion metrics.  Before
    this patch they were overwritten by total values and the original purchase
    metric disappeared from the keyword page.
    """
    if df is None or df.empty:
...
def _build_shopping_terms_keyword_view(shop_terms: pd.DataFrame, meta: pd.DataFrame, _engine=None) -> pd.DataFrame:
    if shop_terms is None or shop_terms.empty:
        return pd.DataFrame()
    work = _perf_common_merge_meta(shop_terms, meta, _engine) if meta is not None and not meta.empty else shop_terms.copy()
    ...
    view["캠페인유형"] = "쇼핑검색"
    query_provided = view.get("query_provided", True)
    query_bucket = view.get("query_bucket", "provided")
    query_raw = view.get("키워드", "")
    query_raw = pd.Series(query_raw, index=view.index).fillna("").astype(str).str.strip()
    unprovided_mask = (
        query_raw.eq("")
        | query_raw.eq("-")
        | query_raw.eq("(검색어 미제공 영역)")
        | pd.Series(query_provided, index=view.index).astype(str).str.lower().isin(["false", "0", "nan"])
        | pd.Series(query_bucket, index=view.index).fillna("").astype(str).str.lower().eq("unprovided")
    )
    ...
    view["구매완료"] = purchase_conv.fillna(0)
    view["전환매출"] = total_sales.where(total_sales > 0, purchase_sales)
    view["구매완료 매출"] = purchase_sales.fillna(0)
    for col in ["매체", "업체명", "담당자", "캠페인", "광고그룹", "키워드"]:
...
def _build_shopping_terms_base_bundle(shop_terms: pd.DataFrame) -> pd.DataFrame:
    ...
    out["purchase_conv"] = purchase_conv.fillna(0)
    out["purchase_sales"] = purchase_sales.fillna(0)
    out["conv"] = total_conv.where(total_conv > 0, purchase_conv)
    out["sales"] = total_sales.where(total_sales > 0, purchase_sales)
    out["imp"] = 0
    out["clk"] = 0
    out["cost"] = 0
    query_raw = out.get("키워드", "")
    query_raw = pd.Series(query_raw, index=out.index).fillna("").astype(str).str.strip()
    query_provided = out.get("query_provided", True)
    query_bucket = out.get("query_bucket", "provided")
    unprovided_mask = (
        query_raw.eq("")
        | query_raw.eq("-")
        | query_raw.eq("(검색어 미제공 영역)")
        | pd.Series(query_provided, index=out.index).astype(str).str.lower().isin(["false", "0", "nan"])
        | pd.Series(query_bucket, index=out.index).fillna("").astype(str).str.lower().eq("unprovided")
    )
...
def _build_shopping_unmapped_conversion_rows(base: pd.DataFrame, detail: pd.DataFrame) -> pd.DataFrame:
    ...
    rows["키워드"] = "(검색어 미제공/기타 영역)"
    rows["구분"] = "쇼핑 미제공 차액"
    rows["전환출처"] = "검색어 미제공/기타(총합-검색어상세)"
...
def _apply_comparison_metrics(view_df: pd.DataFrame, base_df: pd.DataFrame, merge_keys: list) -> pd.DataFrame:
    ...
    agg_dict = {'imp': 'sum', 'clk': 'sum', 'cost': 'sum', 'conv': 'sum', 'sales': 'sum', 'purchase_conv': 'sum', 'purchase_sales': 'sum'}
        
    if not base_df.empty and merge_keys:
        base_agg = base_df.groupby(merge_keys, dropna=False).agg({k: v for k, v in agg_dict.items() if k in base_df.columns}).reset_index()
        ...
        base_agg = base_agg.rename(columns={'imp': 'b_imp', 'clk': 'b_clk', 'cost': 'b_cost', 'conv': 'b_conv', 'sales': 'b_sales', 'purchase_conv': 'b_purchase_conv', 'purchase_sales': 'b_purchase_sales', 'avg_rank': 'b_avg_rank'})
        merged = pd.merge(view_df, base_agg, on=merge_keys, how='left')
    else: merged = view_df.copy()
        
    for c in ['b_imp', 'b_clk', 'b_cost', 'b_conv', 'b_sales', 'b_purchase_conv', 'b_purchase_sales']:
    ...
    c_purchase_conv, b_purchase_conv = merged.get('구매완료', 0), merged.get('b_purchase_conv', 0)
    c_sales, b_sales = merged.get('전환매출', 0), merged.get('b_sales', 0)
    c_purchase_sales, b_purchase_sales = merged.get('구매완료 매출', 0), merged.get('b_purchase_sales', 0)
    ...
    merged['구매완료 증감'], merged['구매완료 차이'] = _vec_pct_diff(c_purchase_conv, b_purchase_conv)
    merged['전환매출 증감'], merged['전환매출 차이'] = _vec_pct_diff(c_sales, b_sales)
    merged['구매완료 매출 증감'], merged['구매완료 매출 차이'] = _vec_pct_diff(c_purchase_sales, b_purchase_sales)
...
def compute_keyword_view(kw_bundle, ad_bundle, meta, _engine=None):
    if (kw_bundle is None or kw_bundle.empty) and (ad_bundle is None or ad_bundle.empty): return pd.DataFrame()
    df_kw = _perf_common_merge_meta(kw_bundle, meta, _engine) if not kw_bundle.empty else pd.DataFrame()
    df_ad = _perf_common_merge_meta(ad_bundle, meta, _engine) if not ad_bundle.empty else pd.DataFrame()
    ...
    if not df_kw.empty:
        rename_dict = {"account_name": "업체명", "manager": "담당자", "platform": "매체", "campaign_type_label": "캠페인유형", "campaign_name": "캠페인", "adgroup_name": "광고그룹", "keyword": "키워드", "imp": "노출", "clk": "클릭", "cost": "광고비", "conv": "전환", "purchase_conv": "구매완료", "sales": "전환매출", "purchase_sales": "구매완료 매출"}
    ...
    if not df_ad.empty:
        rename_dict = {"account_name": "업체명", "manager": "담당자", "platform": "매체", "campaign_type_label": "캠페인유형", "campaign_name": "캠페인", "adgroup_name": "광고그룹", "ad_name": "키워드", "imp": "노출", "clk": "클릭", "cost": "광고비", "conv": "전환", "purchase_conv": "구매완료", "sales": "전환매출", "purchase_sales": "구매완료 매출"}
...
def _keyword_fast_col_config(df: pd.DataFrame, first_col: str = "키워드") -> dict:
    ...
    currency_cols = {"CPC(원)", "CPA(원)", "광고비", "전환매출", "구매완료 매출"} | {c for c in df.columns if c.endswith("차이") and ("광고비" in c or "매출" in c or "CPC" in c)}
    count_cols = {"노출", "클릭", "전환", "구매완료"} | {c for c in df.columns if c.endswith("차이") and c not in currency_cols}
    for c in df.columns:
        if c in cfg:
            continue
        if c in pct_cols:
...
def _keyword_numeric(df: pd.DataFrame, col: str) -> pd.Series:
    return safe_numeric_col(df, col) if col in df.columns else pd.Series([0.0] * len(df.index), dtype="float64")


def _keyword_cutoff(series: pd.Series, quantile: float = 0.75) -> float:
...
def _filter_keyword_workbench(df: pd.DataFrame, preset: str) -> pd.DataFrame:
    if df is None or df.empty:
        return pd.DataFrame() if df is None else df
    work = df.copy()
    cost = _keyword_numeric(work, "광고비")
    clicks = _keyword_numeric(work, "클릭")
    purchase = _keyword_numeric(work, "구매완료")
    total_conv = _keyword_numeric(work, "전환")
    roas = _keyword_numeric(work, "ROAS(%)")
    ...
    if preset == "클릭 많고 구매 0":
        click_cutoff = max(1.0, _keyword_cutoff(clicks, 0.75))
...
def _add_keyword_status(df: pd.DataFrame) -> pd.DataFrame:
    if df is None or df.empty:
        return pd.DataFrame() if df is None else df
    work = df.copy()
    ...
    total_conv = _keyword_numeric(work, "전환")
    roas = _keyword_numeric(work, "ROAS(%)")
    cost_cutoff = max(1.0, _keyword_cutoff(cost, 0.75))
    click_cutoff = max(1.0, _keyword_cutoff(clicks, 0.75))
...
def _render_keyword_workbench_cards(df: pd.DataFrame) -> None:
    if df is None or df.empty:
        return
    cost = _keyword_numeric(df, "광고비")
    purchase = _keyword_numeric(df, "구매완료")
    total_conv = _keyword_numeric(df, "전환")
    ...
    render_ops_cards([
        {"title": "표시 항목", "value": f"{total_rows:,}개", "note": f"구매 발생 {purchase_rows:,}개", "tone": "info", "icon": "KW"},
...
def _keyword_visible_cols(df: pd.DataFrame, base_cols: list[str], metrics_cols: list[str], mode: str) -> list[str]:
    if df is None or df.empty:
        return []
    if mode == "전체 지표":
        cols = []
        ...
        cols = [
            "키워드", "점검", "구분", "전환출처", "매체", "캠페인", "광고그룹",
            "광고비", "클릭", "CPC(원)", "전환", "구매완료", "CPA(원)",
            "전환매출", "구매완료 매출", "ROAS(%)",
        ]
    else:
        cols = [
            "키워드", "점검", "캠페인", "광고그룹", "광고비", "클릭",
            "CPC(원)", "구매완료", "구매완료 매출", "ROAS(%)", "전환", "전환매출",
        ]
    seen = []
    for col in cols:
...
def _keyword_sort_cols_for_preset(preset: str, df: pd.DataFrame) -> list[str]:
    candidates_by_preset = {
        "구매 발생": ["구매완료", "구매완료 매출", "광고비"],
        "비용 발생·구매 0": ["광고비", "클릭"],
        "클릭 많고 구매 0": ["클릭", "광고비"],
        "ROAS 우수": ["ROAS(%)", "구매완료 매출"],
        "총전환 있음": ["전환", "전환매출", "광고비"],
    }
    candidates = candidates_by_preset.get(preset, ["광고비", "전환매출", "전환", "구매완료"])
...
def render_keyword_main(view, top_n):
    ...
    metrics_cols = ["노출", "클릭", "CTR(%)", "CPC(원)", "광고비", "전환", "구매완료", "CPA(원)", "전환매출", "구매완료 매출", "ROAS(%)"]
    ...
    with tool_col_b:
        keyword_col_mode = st.segmented_control(
            ...
            ["핵심", "전환 상세", "전체 지표"],
            default="핵심",
            key="kw_col_mode_main",
        )

    disp = _filter_keyword_workbench(disp, keyword_preset)
    if disp.empty:
        st.info("선택한 업무 보기 조건에 맞는 키워드/소재가 없습니다.")
        return
    disp = _add_keyword_status(disp)
    _render_keyword_workbench_cards(disp)

    final_cols = _keyword_visible_cols(disp, base_cols, metrics_cols, keyword_col_mode)
    sort_cols = _keyword_sort_cols_for_preset(keyword_preset, disp)
    ...
    _render_export_buttons(disp, "키워드_소재_종합성과", "kw_main_export")
...
def render_keyword_cmp(view_orig, engine, cids, type_sel, top_n, start_dt, end_dt):
    ...
    if agg_kw_cmp:
        ...
        if not base_bundle.empty:
            ...
            base_agg_dict = {'imp': 'sum', 'clk': 'sum', 'cost': 'sum', 'conv': 'sum', 'sales': 'sum', 'purchase_conv': 'sum', 'purchase_sales': 'sum'}
            base_bundle = base_bundle.groupby(base_grp_cols, as_index=False, dropna=False).agg({k: v for k, v in base_agg_dict.items() if k in base_bundle.columns})
    ...
    metrics_cols_cmp.extend(["구매완료", "구매완료 증감", "구매완료 차이"] if show_deltas else ["구매완료"])
    metrics_cols_cmp.extend(["CPA(원)"])
    metrics_cols_cmp.extend(["전환매출", "전환매출 증감", "전환매출 차이"] if show_deltas else ["전환매출"])
    metrics_cols_cmp.extend(["구매완료 매출", "구매완료 매출 증감", "구매완료 매출 차이"] if show_deltas else ["구매완료 매출"])
    ...
    sort_cols_cmp = [c for c in ["광고비", "전환매출", "전환", "구매완료"] if c in disp_cmp.columns]
    ...
    _render_export_buttons(disp_final, "키워드_기간비교", "kw_cmp_export")
...
def page_perf_keyword(meta: pd.DataFrame, engine, f: Dict) -> None:
    ...
    if view is not None and not view.empty:
        ...
        purchase_conv = float(safe_numeric_col(view, "구매완료").sum()) if "구매완료" in view.columns else 0.0
