...
def _build_type_performance_summary(cur_camp: pd.DataFrame) -> pd.DataFrame:
    columns = [
    ...
    type_col = _campaign_type_column_from_frame(cur_camp)
    if not type_col:
        return pd.DataFrame(columns=columns)
    ...
    grouped = work.groupby("_type_label", as_index=False, dropna=False)[
...
def page_overview(meta: pd.DataFrame, engine, f: Dict) -> None:
    ...
    type_perf_summary = _build_type_performance_summary(type_snapshot_camp)
